...
def index(request):
    if request.method == 'POST':
        ...
        if ffarmer != 'all' and fproduct !='all':
            u = User.objects.get(username=ffarmer)
            ud = User_details.objects.get(user = u)
            f_products = Products.objects.filter(product_name = fproduct, seller = ud )
        elif ffarmer == 'all' and fproduct == 'all' and fdist == 'all':
        ...
        if (fdist) == 'all':
            f_dist = 0
            return render(request,'pages/home.html',{'dist':f_dist,'fp':fproduct,'ff':ffarmer,'products':f_products,'farmers':farmers,'d_products':d_products,})
        else:
